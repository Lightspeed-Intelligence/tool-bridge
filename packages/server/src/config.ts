...
export function configFromEnv(env: NodeJS.ProcessEnv = process.env): ServerConfig {
  ...
  // 飞书登录:配了 secret ref 才启用(缺省不开)。toolCacheTtlSec / refThresholdBytes /
  // refTtlSec 等共用字段已由 core parseRuntimeEnv 解析,此处只保留 fork 定制字段。
  ...
  const loginTtl = parsePositiveIntEnv(env.TB_FEISHU_LOGIN_KEY_TTL_SEC)
