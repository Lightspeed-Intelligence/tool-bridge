...
- 2026-07-29:**bytebase 第二个挂载 `plugins/bytebase-rw`(权限分级)上线并实测边界**——同一 plugin 部署 + 第二份凭证(SA `tool-bridge-rw@service.bytebase.com`,SecretStore `bytebase-rw`),用途:**test 环境直接改 + 生产只能提工单**。授权三段:工作区级 `sqlEditorReadUser`(全环境可读,写迁移前得能看生产 schema)+ 工作区级 `projectDeveloper`(`bb.sheets.create`/`plans.create`/`issues.create` → 提工单);**每个项目**级 `sqlEditorUser` 带 CEL 条件 `resource.environment_id in ["test"]`(已授全部 5 个项目:default / new-api-tdis / tipsy-backend-131q / tipsy-memory / tipsystudio)。**关键约束**:CEL 条件绑定**只能下在项目级**(proto 明写 condition "only used in the project IAM policy"),故新建项目须手动补这条,否则该项目 test 库不可写;提工单的三个权限是项目/工作区级、**无法按环境区分**,所以"生产只提单不直写"靠的是写权限的 CEL 条件而非工单权限本身。**边界实测(全部经网关→plugin 真实链路)**:生产读 `SELECT 1` on studiorprod/studio 通;生产直写 `CREATE TABLE` 被拒(`permission_denied: permission denied to access resources`)——CEL 生效;test 环境 `CREATE TABLE` + `INSERT` + 读回 + `DROP TABLE` 全通(探针表已清理,`information_schema` 复查为 0);生产提工单成功(`projects/tipsystudio/issues/177`,rollout task 状态 `NOT_STARTED` = SQL 从未执行,生产库复查无任何探针表)。**遗留**:该探针工单状态卡在 `DONE` —— `UpdateIssue{status:CANCELED}` 被服务端改写为 `DONE` 且 `approvalStatus:SKIPPED`(项目配了 `requireIssueApproval:true` 但 riskLevel=LOW 触发跳过审批);工单无害(目标表不存在 + DDL simulation 已 WARNING)但状态语义不对,需人工在 UI 关闭。
- 2026-07-29:**plugin-bytebase 已部署已注册已挂载并生产复验通过**——Worker `tb-plugin-bytebase` @ https://tb-plugin-bytebase.heco.workers.dev(Lightspeed 账户,version `397c98fd-52b6-4807-83d9-cee9b18daf35`);生产网关 `tb plugin register`(id=bytebase,platform-token)+ 挂载树节点 `plugins/bytebase`(kind:tool,authRef=bytebase-sa;初次挂在根下,同日归位到 `plugins/`)。SA `tool-bridge@service.bytebase.com` 授 **`roles/sqlEditorReadUser`(只读)**;`BYTEBASE_ALLOWED_TOOLS` 留空(放行上游全部 6 工具,写操作由 Bytebase IAM 拦)。**全链路复验**(网关→plugin→SA 换发→Bytebase MCP):`tb help plugins/bytebase` 列出 6 工具且 effect 正确;`search_api`、`query_database`(`SELECT 1` on instances/tipsy-memory/databases/tipsy_memory,返回 1 行)通;`propose_database_change` 被 Bytebase 侧拒(`PERMISSION_DENIED: you don't have permission to create sheets`)——只读边界经实测生效。**过程中发现并修掉一个自己的凭证隔离 bug**:令牌缓存键原为 `<baseUrl>|<email>`,错 service_key 会命中同 email 的有效缓存回 200(泄漏 PLUGIN_TOKEN + 猜到 SA 邮箱即可白蹭令牌,且 key 轮换在旧令牌到期前不生效);已改为键含 `sha256(service_key)` 前 16 hex(MCP 会话键同粒度),加回归用例(14 例)并重部署,生产复测错 key → 503。
- 2026-07-29:**plugin-bytebase 新包**——Bytebase 自托管 MCP 的 tool-provider/v1 plugin(CF Worker),动机:Bytebase `/mcp` 只认 OAuth bearer 且 DCR 白名单只放行 loopback,直挂 `kind:mcp` 每次得 `tb tool auth --local` 开浏览器授权(见本文 2026-07-08 托管 OAuth 条的严格上游降级通道),token 1h 过期、refresh 轮换并发下互相作废。改用**服务账号**:`POST /v1/auth/login` 以 `service_key` 作 password 换发访问令牌(audience `bb.user.access`,**固定 1h**,响应无 expires_in → 到期时刻从 JWT `exp` 解),isolate 内存缓存至余量 5min,上游 401 强制重换发重试一次——平台侧呈现为永不过期、零人工授权。凭证边界同 plugin-feishu(email/service_key 存平台 SecretStore,经挂载 `authRef` → `X-TB-Upstream-Auth` 注入,plugin 无凭证即空壳);缓存与 MCP 会话按 `<baseUrl>|<email>|sha256(service_key)前16hex` 键控(多实例/多账号不串号,且错 key 不蹭缓存)。**实证(2026-07-29,生产实例 https://bytebase.infra.fantacy.live,Bytebase 3.19.0)**:SA login → token → `/mcp` initialize + tools/list 全通,宣告 6 工具(call_api / get_schema / get_skill / propose_database_change / query_database / search_api);**上游 tools/list 不带 annotations**(只有 name/description/inputSchema)→ plugin 按工具名兜底 effect,否则 destructive 二次确认失效;失效信号分层实测:坏 token → 401(带 RFC 9728 WWW-Authenticate),有效 token + 陈旧 sessionId → 404。14 例集成测试全绿,`pnpm verify` 退出码 0。上线与复验见上一条。
...
- `packages/gateway` — Workers 胶水(可发布 Worker library:tsup 单文件 ESM bundle core,`src/index.ts` 另 export `createApp` 与 `type Env`;dev exports 指 src(含 `./deviceHello`)、发布形态 publishConfig 覆盖指 dist),**131 个默认集成测试 + 6 个 opt-in skipped**(真实 workerd;含 mcp 过期会话空列表防御、自定义认证头与托管 OAuth 全链路的 mock 上游用例、system/federation 运行时白名单叠加用例、annotation/~feedback、skillhub 与安全回归用例):`app.ts`(Env→deps 适配)/ `tbApp.ts`(**宿主中立 createTbApp**,路由/认证/安全头/HTBP/remote 聚合,SDK 复用;remote 白名单经 `resolveRemoteSettings` 取 env 基线 ∪ 运行时条目)/ `bootstrap.ts` / `deviceHello.ts`(宿主中立 processDeviceHello,DO 与 Node DeviceHub 共用)/ `oauth.ts`(mcp 托管 OAuth:加密 state + OAuthClientProvider + 发起/回调编排)/ `kvStateStore.ts` / `refToken.ts`(`$ref` 中转 token)/ `deviceSession.ts`(DeviceSession DO 胶水)/ `providers/`(mcp / http / remote / pluginTool / pluginContext / pluginClient / r2Object / s3Object / s3Sign / toolCache);`wrangler.jsonc` 在此包内。
...
- `packages/plugin-bytebase` — Bytebase tool-provider plugin(private,CF Worker),**14 个集成测试**(真实 workerd,mock login 与 MCP 上游,默认离线):SA 令牌换发与缓存命中、401 强制重换发自愈、会话 404 重握手不误触发换发、凭证头缺失/坏形状/缺字段、错 service_key 只打一次 login(避开登录锁定)、错 key 不蹭同 email 缓存令牌(生产实测回归)、白名单双闸(List 过滤 + Call 拒绝)、多账号按缓存键不串号、base_url 覆盖与归一、`decodeExp`;文件职责与权限/审计边界见 code-map。
- `packages/server` — Node/Docker 宿主胶水(可发布 bin `tool-bridge-server`,0.1.0),**24 个测试**(5 文件,纯 Node vitest):`sqliteStateStore.ts`(better-sqlite3 单表 kv,WAL,强一致)/ `config.ts`(configFromEnv,TB_PORT/TB_HOST/TB_DATA_DIR/TB_UI_DIR)/ `objects.ts`(FsObjectStore 前缀适配)/ `deviceHub.ts`(ws 设备通道,复用 core DeviceGatewaySession + gateway processDeviceHello,invoke 前重验 SK)/ `assets.ts`(/ui 静态托管)/ `server.ts` + `main.ts`;tsup bundle core+gateway,better-sqlite3/ws/hono 等留 external;根 Dockerfile 产出镜像。详见 [../guides/docker-host.md](../guides/docker-host.md)。
- `scripts/` — `gen-dev-vars.mjs` / `provision.mjs` / `smoke.ts`(healthz + 无 SK 401 + 带 SK 200)+ 三个可重跑生产验收脚本:`verify-revocation.ts`(吊销传播,默认观察预算 120s,可配置;历史实测 0.3s)/ `verify-device.ts`(设备 shell/fs/registerPaths 全链路,可选跨休眠用例)/ `verify-plugin.ts`(Plugin 注册→挂载→四动词全流程)。
...
- **端到端验收系统化**:七个 User Case 的脚本化 E2E(CLI `--json` 驱动 + Dashboard 浏览器侧)未拉通;现有 smoke + 三个 verify-* 脚本覆盖主链路。Dashboard 0.6.0 有高价值真实浏览器终验,但仍缺可重跑回归来固化树 depth/localize 与 ARIA 键盘状态、workspace 卸载后的 mutation/invalidation、Plugin 并发行操作、一次性值/待决 dialog,以及 cursor 边界上的冲突与 replace/upsert 警告。安全面后续需单独设计 HttpOnly session+CSRF+BFF 与分布式 rate limit;不要用 sessionStorage/浏览器端加密或 isolate 内 Map 作为替代。
